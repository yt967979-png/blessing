    ...
    es.onmessage = (event) => {
      try {
        ...
        if (data.type === 'NEW_MESSAGE') {
          setMessages((prev) => {
            if (prev.some((m) => m.id === data.id || (m.text === data.text && m.sender_type === data.senderType))) {
              return prev;
            }
            return [
              ...prev,
          ...
          if (!isOpen && data.senderType !== 'CUSTOMER') {
            ...
            <div className="px-3 py-2 bg-white border-t border-slate-100 flex gap-1.5 overflow-x-auto no-scrollbar shrink-0">
              {QUICK_SUGGESTIONS.map((chip) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (chip.includes('Talk to Admin')) {
                      handleEscalateToHuman();
                    } else {
                      handleSendMessage(chip);
                    }
                  }}
                  className="whitespace-nowrap px-2.5 py-1 bg-slate-100 hover:bg-blue-50 hover:text-[#2874f0] text-slate-600 rounded-full text-[10.5px] font-bold border border-slate-200 transition-colors cursor-pointer shrink-0"
                >
                  {chip}
                </button>
              ))}
