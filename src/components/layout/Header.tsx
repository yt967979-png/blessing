    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4 md:gap-8">
              ...
              {filteredSearch.length > 0 ? (
                <div className="divide-y divide-slate-100">
                  {filteredSearch.map((p) => (
                    ...
                    >
                      <img src={p.image} alt={p.title} className="w-8 h-8 object-contain rounded bg-slate-100 p-0.5" />
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-[#001B3A] truncate">{p.title}</div>
                        <div className="text-[10px] text-slate-500">{p.cls} • ₹{p.price}</div>
                      </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {/* Wishlist Button */}
          <button
            onClick={() => alert(`${wishlist.length} items saved in your wishlist`)}
