...
export default function ProductDetailClient({ slug, initialProduct }: { slug: string; initialProduct?: any }) {
  ...
  const [reviewRole, setReviewRole] = useState<'Parent' | 'Student' | 'Teacher'>('Parent');
  const [reviewDistrict, setReviewDistrict] = useState('Madurai');
    ...
    try {
      ...
      const res = await fetch('/api/reviews', {
        ...
        body: JSON.stringify({
          ...
          comment: reviewText,
          images: reviewImages,
          district: reviewDistrict,
          reviewerType: reviewRole,
          studentClass: product?.cls ? `${product.cls} Standard` : '10th Standard',
        }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col page-mobile-nav-pdp md:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 py-5 sm:py-8 flex-1">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 md:p-8 shadow-xs grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 mb-12">
          ...
          <div className="lg:col-span-7 flex flex-col">
            ...
            {/* Sample PDF Preview & Direct WhatsApp Sample Action */}
            <div className="mb-4 p-3 rounded-2xl bg-gradient-to-r from-purple-50/70 via-blue-50/50 to-emerald-50/60 border border-slate-200/90 shadow-2xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-black uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
                  <FileText className="w-3.5 h-3.5 text-purple-600" />
                  <span>Sample Chapter Preview</span>
                </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {product.samplePdfUrl && (
                  <button
                    ...
                    className="py-2.5 px-3 bg-white hover:bg-slate-50 text-[#001B3A] border border-slate-200/90 rounded-xl text-xs font-black flex items-center justify-between gap-2 transition-all cursor-pointer shadow-2xs group"
                    title="Read sample chapter in-app with zoom & flipbook"
                  >
                    <div className="flex items-center gap-2 text-left truncate">
                      <div className="w-7 h-7 rounded-lg bg-[#001B3A] text-amber-400 flex items-center justify-center shrink-0 shadow-2xs">
                        <BookOpen className="w-3.5 h-3.5" />
                      </div>
                      <div className="truncate">
                        <div className="font-extrabold text-[#001B3A] truncate">Preview Sample PDF</div>
                        <div className="text-[10px] text-slate-500 truncate">Read in-app chapters</div>
                      </div>
                    </div>
                    <span className="text-[10px] font-extrabold px-2 py-1 bg-slate-100 rounded-lg text-slate-700 shrink-0">
                      Open ↗
                    </span>
                  </button>
                )}
                ...
                <a
                  href={samplePdfWhatsAppUrl(product.title, product.cls)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`py-2.5 px-3 bg-[#25D366]/15 hover:bg-[#25D366]/25 text-[#128C7E] border border-[#25D366]/30 rounded-xl text-xs font-black flex items-center justify-between gap-2 transition-all cursor-pointer shadow-2xs group ${
                    !product.samplePdfUrl ? 'sm:col-span-2' : ''
                  }`}
                  title="Ask for book sample chapter PDF directly on WhatsApp"
                >
                  <div className="flex items-center gap-2 text-left truncate">
                    <div className="w-7 h-7 rounded-lg bg-[#25D366] text-white flex items-center justify-center shrink-0 shadow-2xs">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                        <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
                      </svg>
                    </div>
                    <div className="truncate">
                      <div className="font-extrabold text-[#128C7E] truncate">Get Sample on WhatsApp</div>
                      <div className="text-[10px] text-emerald-700/90 truncate">Direct PDF to your phone</div>
                    </div>
                  </div>
                  <span className="text-[10px] font-extrabold px-2 py-1 bg-white rounded-lg text-[#128C7E] border border-[#25D366]/30 shrink-0">
                    💬 Chat
                  </span>
                </a>
              </div>
            ...
            <div className="bg-blue-50/50 border border-blue-200 rounded-xl p-5 mb-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={reviewRole}
                    onChange={(e: any) => setReviewRole(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs outline-none focus:border-blue-600 font-bold bg-white"
                  >
                    <option value="Parent">Parent (Class {product?.cls || '10th'})</option>
                    <option value="Student">Student (Class {product?.cls || '10th'})</option>
                    <option value="Teacher">Subject Teacher / Tutor</option>
                  </select>
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-700 block mb-1">Your District / City in Tamil Nadu *</label>
                  <input
                    type="text"
                ...
                <div key={rev.id || rev.studentName + rev.comment} className="bg-slate-50 border border-slate-200 rounded-xl p-5 flex flex-col justify-between hover:border-slate-300 transition-colors">
                  <div>
                    <div className="flex justify-between items-start mb-2.5 gap-2">
                      <div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-extrabold text-slate-900 text-sm">{rev.studentName}</span>
                          <span className="text-slate-300 font-bold">•</span>
                          <span className="text-[11px] font-bold text-blue-900 bg-blue-50 border border-blue-200/90 px-2 py-0.5 rounded-full inline-flex items-center gap-1 shadow-2xs">
                            <span>🎓</span>
                            <span>
                              {rev.badgeText || (
                                `${rev.studentClass || `${product?.cls || 'Class 10'}`} ${rev.reviewerType || 'Parent'}, ${rev.district || 'Madurai'}`
                              )}
                            </span>
                          </span>
                          {rev.verifiedPurchase && (
                            <span className="text-[9px] font-black text-emerald-800 bg-emerald-100/90 px-1.5 py-0.5 rounded border border-emerald-300 uppercase tracking-wide">
                              ✓ Verified Buyer
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="flex gap-0.5 shrink-0 bg-amber-50 border border-amber-200/60 px-2 py-1 rounded-lg">
      ...
      {/* Mobile Sticky Bottom Action Bar ("Buy Now") — Flipkart / Amazon standard */}
      <div
        className="fixed inset-x-0 bottom-0 z-50 sm:hidden bg-white/98 backdrop-blur-md border-t border-slate-200 shadow-[0_-6px_25px_rgba(0,27,58,0.12)] p-2.5 pb-[max(0.65rem,env(safe-area-inset-bottom))] transition-all duration-300"
      >
        <div className="flex items-center gap-2 max-w-lg mx-auto">
          ...
          <button
            type="button"
            onClick={() => toggleWishlist(product.id)}
            className="w-11 h-11 rounded-xl border border-slate-200/90 text-slate-700 bg-slate-50 flex items-center justify-center shrink-0 touch-manipulation active:scale-95 transition-transform"
            aria-label="Wishlist"
          >
            <Heart className={`w-4 h-4 transition-colors ${isWishlisted ? 'text-red-500 fill-red-500' : 'text-slate-400'}`} />
          </button>
          ...
          <button
            type="button"
            onClick={shareViaWhatsApp}
            className="w-11 h-11 rounded-xl border border-emerald-200 text-[#25D366] bg-emerald-50 flex items-center justify-center shrink-0 touch-manipulation active:scale-95 transition-transform"
          ...
          {/* Price & Savings Telemetry */}
          <div className="flex flex-col justify-center min-w-0 px-1">
            <div className="flex items-baseline gap-1 leading-none">
              <span className={`font-black text-base ${product.inStock === false ? 'text-slate-400' : 'text-[#001B3A]'}`}>
                ₹{product.price}
              </span>
              {product.mrp > product.price && (
            ...
            <div className="text-[9px] font-black mt-0.5 truncate">
              {product.inStock === false ? (
                ...
                <span className="text-emerald-700 font-extrabold">{product.discount}% OFF</span>
              ) : (
          ...
          {product.inStock === false ? (
            ...
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              <button
                ...
                className={`font-black text-[11px] py-3 px-3 rounded-xl flex items-center justify-center gap-1 uppercase tracking-wider touch-manipulation min-h-11 transition-all duration-300 cursor-pointer ${
                  isMobileAdded
                    ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/30 flex-1'
                    : 'bg-[#0044AA] hover:bg-[#003388] active:bg-[#001B3A] text-white flex-1'
                }`}
              >
                {isMobileAdded ? (
                  <>
              ...
              <button
                type="button"
                onClick={tryBuyNow}
                className="flex-[1.3] bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 active:from-amber-500 active:to-amber-700 text-[#001B3A] font-black text-xs py-3 px-3 rounded-xl uppercase tracking-wider touch-manipulation shadow-md shadow-amber-500/20 active:scale-95 transition-all flex items-center justify-center gap-1 cursor-pointer min-h-11"
              >
                <Zap className="w-3.5 h-3.5 fill-[#001B3A] text-[#001B3A]" />
                <span>BUY NOW</span>
              </button>
            </div>
          )}
