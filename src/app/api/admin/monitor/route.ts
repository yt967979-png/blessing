...
export async function GET(request: NextRequest) {
  ...
  // 6. E-Commerce Today (cached in Redis for 30 seconds to prevent DB churn)
  let ecommerceToday = {
    orders: 0,
    revenue: 0,
    booksSold: 0,
  ...
  // 7. System Alerts Rule Engine
  const alerts: Array<{ level: 'CRITICAL' | 'WARNING' | 'INFO'; message: string; timestamp: string }> = [];

  if (diskStats.usedPercent >= 85) {
    alerts.push({
      level: 'CRITICAL',
      message: `NVMe storage reached ${diskStats.usedPercent}% (${Math.round(diskStats.usedBytes / 1024 / 1024 / 1024)}GB / ${Math.round(diskStats.totalBytes / 1024 / 1024 / 1024)}GB). Run backup cleanup.`,
