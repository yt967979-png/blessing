  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-[#55607A]/20 p-4 shadow-xs space-y-3">
        {/* Status Filter Tabs */}
        <div className="flex flex-wrap gap-1.5 border-b border-slate-100 pb-3 text-xs font-mono">
          {[
            { key: 'all', label: 'ALL ORDERS', count: counts.all },
            { key: 'pending', label: 'UNPACKED', count: counts.pending, color: 'text-[#0284c7]' },
            { key: 'packed', label: 'PACKED', count: counts.packed, color: 'text-[#D98C2B]' },
            { key: 'dispatched', label: 'IN TRANSIT', count: counts.dispatched, color: 'text-[#1E2A4A]' },
            { key: 'delivered', label: 'DELIVERED', count: counts.delivered, color: 'text-[#2F9E60]' },
            { key: 'cancelled', label: 'CANCELLED', count: counts.cancelled, color: 'text-[#C43B3B]' },
          ].map((tab) => (
            <button
              key={tab.key}
              type="button"
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                statusFilter === tab.key
                  ...
                  : 'bg-[#FAF7F0] text-[#55607A] hover:bg-slate-200/70'
              }`}
            >
              <span>{tab.label}</span>
              <span
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-[#FAF7F0] border border-[#55607A]/20 rounded-lg text-xs font-mono outline-none focus:border-[#D98C2B] focus:ring-1 focus:ring-[#D98C2B]"
            />
          </div>

          {/* Batch Action Buttons (When items selected) */}
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-2 bg-[#FAF7F0] p-1 rounded-lg border border-[#D98C2B]/40 animate-fade-slide-up">
              <span className="text-xs font-mono font-bold text-[#1E2A4A] px-2">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-[#D98C2B] hover:bg-[#c27c24] text-white rounded text-xs font-mono font-bold flex items-center gap-1 transition-colors cursor-pointer shadow-xs"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white border border-[#55607A]/30 text-[#1E2A4A] hover:bg-slate-100 rounded text-xs font-mono font-bold flex items-center gap-1 transition-colors cursor-pointer shadow-xs"
              >
