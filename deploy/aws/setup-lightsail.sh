...
echo "  4. After secrets: sudo bash deploy/aws/redeploy.sh '$REPO_DIR'  (or systemctl start if already built)"
echo "  5. Verify /api/health and /api/ready — keep Railway up until AWS.md §6 / §11 pass"
echo "  6. Later updates: git pull in clone, then sudo bash deploy/aws/redeploy.sh <clone>"
echo ""
echo "Do not start blessing until secrets are real. Prefer redeploy.sh over hand-rolled restart."
